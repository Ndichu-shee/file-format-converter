...
def read_data_from_csv(file, schema):
    ...
    all_data_frame = pd.read_csv(
        file, names=all_columns
    )  # convert a dict to a dataframe(rows and column) path, header, names(column names)
...
def convert_to_json(all_data_frame, data_set_name, file_name, target_dir):
    ...
    os.makedirs(
        f"{target_dir}{data_set_name}", exist_ok=True
    )  # creates a directory pathe, exists_ok(if True it does not throw an error)if it does not exists
    all_data_frame.to_json(
        file_path, orient="records", lines=True
    )  # loads rows and column to json data/string path, orient, lines
